...
| **An out-of-memory inside JSON parsing is reported as `error.TypeMismatch`** | `sql/scan.zig`'s JSON paths do `std.json.parseFromSliceLeaky(...) catch return error.TypeMismatch`, so an allocation failure while parsing a JSON column arrives as a *value* problem — and in the lenient scanners it is masked into the field's default, i.e. an OOM becomes data. Narrowing is one switch (`error.OutOfMemory => error.OutOfMemory, else => error.TypeMismatch`), and it is also what unblocks the arena half of the allocation sweep (see the structural gap above). Not done because it changes an error contract for a column type nothing in-tree exercises, so it wants a consumer's read on whether anything switches on that error |
...
| Gap | Why it matters |
|---|---|
| **The allocation-failure sweep still has known gaps** | `src/test/allocation_failures*.zig` use `std.testing.checkAllAllocationFailures` (Zig 0.17) to fail every allocation in turn and hold the byte ledger; they have found fourteen leaks so far (three in `sql/builder.zig` at v0.76.1, eight in `sql/scan.zig` and `sql/schema/migrate.zig` at v0.77.0). Covered today: fragment rendering, an assembled SELECT, the row scanners, `queryAll`, `planMigrateStatements`, `CrudService.getOwned`, and the neighbour fragments (clean). Uncovered, in the order they are most likely to hide something: (**a**) the JSON/arena half of the scanners — blocked by the defect below, since an OOM inside `std.json` is reported as `error.TypeMismatch` and the sweep fails on any non-OOM error; (**b**) `getMySQLIndexes` / `getPostgresIndexes`, whose leaks were fixed by inspection because the stub's catalog is SQLite-only — they need a per-dialect stub; (**c**) `Builder.init`, which *swallows* an induced OOM by design, so the sweep reports `SwallowedOutOfMemoryError` and skips it (`initCapacity` is swept instead) |
