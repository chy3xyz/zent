...
| Gap | Why it matters |
|---|---|
| **The allocation-failure sweep covers three paths out of many** | `src/test/allocation_failures.zig` uses `std.testing.checkAllAllocationFailures` (Zig 0.17) to fail every allocation in turn and hold the byte ledger; it found three unconditional OOM-path leaks in `sql/builder.zig` on its first run (v0.76.1). Still uncovered, in the order they are most likely to hide one: `crud.zig`'s `ownedCopy` partial-dupe teardown, `migrate.planMigrateStatements`'s statement list, `query.zig`'s entity scan (`scanRowWithArena` + the per-entity `errdefer`), and `graph/neighbors.zig`'s fragment assembly. Each needs a file-scope schema (the caller must be a plain fn), which is why the sweep lives in its own file |
