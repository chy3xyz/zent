...
| Z24 | `PoolExhausted` flattens every failure; no request-level borrow budget | P0 | **Fixed** v0.51.0 (taxonomy) + v0.56.0 (budget, `PoolWaitTimeout`) |
...
| Z28 | raw identifiers have no validation entry point; introspection is private | P2 | **Half fixed** v0.53.0 — introspection + `checkSchema` exported; `checkStatement` **Open** |
...
| Z31 | `migrateSchema` creates the drift it later warns about | P2 | **Partly fixed** v0.56.0 — nullability behind `allow_nullability_change`; `ExistingIndex.columns`, views, `UNIQUE`/FK **Open** |
