...
| 5, 6, 8, 9, 12, 13 | Arena scanning, public introspection, migration self-drift, `CrudService` tenant column, sub-query scope, error classification, pool observability | **Read, not re-verified line by line.** #10 (sub-query predicates do not scope the inner table) shares the shape of Z25 and is worth its own pass; #5 is the largest engineering ask in the list. |
