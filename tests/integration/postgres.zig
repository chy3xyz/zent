...
test "Postgres: eager-load interceptor scope is unambiguous on JOIN edges (m2o + m2m)" {
    ...
    // same tenant column as the target, an unqualified interceptor EQ is
    // rejected by PostgreSQL as an ambiguous column reference.
    const allocator = testing.allocator;
    var drv = connect(allocator) catch |err| return skipIfNoServer(err);
    defer drv.close();

    const FileBase = schema("PgXjFile", .{
        .fields = &.{ field.String("path"), field.Int("app_id") },
    });
    const ImageBase = schema("PgXjImage", .{
        .fields = &.{
    ...
    const TagBase = schema("PgXjTag", .{
        .fields = &.{ field.String("label"), field.Int("app_id") },
    });
    const PostBase = schema("PgXjPost", .{
        .fields = &.{ field.String("title"), field.Int("app_id") },
    });
    const PostTag = schema("PgXjPostTag", .{
        .fields = &.{
            field.Int("pg_xj_post_id"),
            field.Int("pg_xj_tag_id"),
            field.Int("app_id"),
        },
        .indexes = &.{index.Fields(&.{ "pg_xj_post_id", "pg_xj_tag_id" }).Unique()},
    });
    ...
    for (&file_ids, 0..) |*out, i| {
        var b = try client.pg_xj_file.Create();
        defer b.deinit();
    ...
    for ([_]struct { caption: []const u8, app: i64, file: usize }{
        ...
        var b = try client.pg_xj_image.Create();
        defer b.deinit();
    ...
    {
        var q = client.pg_xj_image.Query();
        defer q.deinit();
    ...
    for (&tag_ids, 0..) |*out, i| {
        var b = try client.pg_xj_tag.Create();
        defer b.deinit();
    ...
    for (&post_ids, 0..) |*out, i| {
        var b = try client.pg_xj_post.Create();
        defer b.deinit();
    ...
    for ([_]struct { post: usize, tag: usize }{
        ...
        _ = try drv.exec(
            "INSERT INTO pg_xj_post_tag (pg_xj_post_id, pg_xj_tag_id, app_id) VALUES ($1, $2, $3)",
            &.{
                .{ .int = post_ids[l.post] },
                .{ .int = tag_ids[l.tag] },
...
test "Postgres: Update edge writes maintain M2M and O2M associations" {
    const allocator = testing.allocator;
    var drv = connect(allocator) catch |err| return skipIfNoServer(err);
    defer drv.close();

    const CarBase = schema("PgEwCar", .{
        .fields = &.{ field.String("model"), field.Int("owner_id").Optional() },
    });
    const GroupBase = schema("PgEwGroup", .{
        .fields = &.{field.String("name")},
    });
    const UserBase = schema("PgEwUser", .{
        .fields = &.{field.String("name")},
    });
    const UserGroup = schema("PgEwUserGroup", .{
        .fields = &.{ field.Int("pg_ew_user_id"), field.Int("pg_ew_group_id") },
        // Explicit through schemas need their own uniqueness guarantee for
        // idempotent `INSERT ... ON CONFLICT DO NOTHING`.
        .indexes = &.{index.Fields(&.{ "pg_ew_user_id", "pg_ew_group_id" }).Unique()},
    });
    const Car = struct {
        pub const schema_name = CarBase.schema_name;
        pub const fields = CarBase.fields;
    ...
    _ = try drv.exec("DROP TABLE IF EXISTS pg_ew_user_group CASCADE", &.{});
    _ = try drv.exec("DROP TABLE IF EXISTS pg_ew_car CASCADE", &.{});
    _ = try drv.exec("DROP TABLE IF EXISTS pg_ew_group CASCADE", &.{});
    _ = try drv.exec("DROP TABLE IF EXISTS pg_ew_user CASCADE", &.{});
    try Client.createAllTables(std.testing.allocator, infos, drv.asDriver());
    defer {
        _ = drv.exec("DROP TABLE IF EXISTS pg_ew_user_group CASCADE", &.{}) catch {};
        _ = drv.exec("DROP TABLE IF EXISTS pg_ew_car CASCADE", &.{}) catch {};
        _ = drv.exec("DROP TABLE IF EXISTS pg_ew_group CASCADE", &.{}) catch {};
        _ = drv.exec("DROP TABLE IF EXISTS pg_ew_user CASCADE", &.{}) catch {};
    }
    ...
    for (&uids, 0..) |*out, i| {
        var b = try client.pg_ew_user.Create();
        defer b.deinit();
    ...
    for (&gids, 0..) |*out, i| {
        var b = try client.pg_ew_group.Create();
        defer b.deinit();
    ...
    for (&cids, 0..) |*out, i| {
        var b = try client.pg_ew_car.Create();
        defer b.deinit();
    ...
    const preds = client.pg_ew_user.predicates;

    // M2M Add is idempotent: applying the same ids twice keeps two rows.
    for (0..2) |_| {
        var u = client.pg_ew_user.Update();
        defer u.deinit();
    ...
    {
        var groups = try client.pg_ew_user.QueryEdge("groups", &.{uids[0]});
        defer {
            for (groups.items) |*e| zent.codegen.deinitEntity(infos, group_info, e, allocator);
            groups.deinit();
        }
    ...
    {
        var groups = try client.pg_ew_user.QueryEdge("groups", &.{uids[1]});
        defer {
            for (groups.items) |*e| zent.codegen.deinitEntity(infos, group_info, e, allocator);
            groups.deinit();
        }
    ...
    try expectPgCarOwner(&client, infos, car_info, cids[0], null);
    try expectPgCarOwner(&client, infos, car_info, cids[2], uids[0]);
    try expectPgCarOwner(&client, infos, car_info, cids[1], uids[1]);
    ...
    try expectPgCarOwner(&client, infos, car_info, cids[2], null);
    try expectPgCarOwner(&client, infos, car_info, cids[1], uids[1]);
    ...
    try expectPgCarOwner(&client, infos, car_info, cids[1], null);

    // A `From` edge (pg_ew_car.owner) writes its FK column in the UPDATE's own
    // SET clause — no companion `setFieldValue`, no detach statement, and the
    // same predicate scopes it. See Z15.
    {
        var u = client.pg_ew_car.Update();
        defer u.deinit();
        _ = try u.Where(.{client.pg_ew_car.predicates.idEQ(.{ .int = cids[0] })});
        _ = try u.SetEdgeIDs("owner", &.{uids[0]});
        try testing.expectEqual(@as(usize, 1), try u.Save());
    }
    try expectPgCarOwner(&client, infos, car_info, cids[0], uids[0]);
    ...
    try expectPgCarOwner(&client, infos, car_info, cids[0], uids[1]);
    {
        var u = client.pg_ew_car.Update();
        defer u.deinit();
        _ = try u.Where(.{client.pg_ew_car.predicates.idEQ(.{ .int = cids[0] })});
...
fn expectPgCarOwner(
    ...
    var q = client.pg_ew_car.Query();
    defer q.deinit();
    _ = try q.Where(.{client.pg_ew_car.predicates.idEQ(.{ .int = car_id })});
...
test "Postgres: outbox claim is exclusive and requeue re-enables a row" {
    const allocator = testing.allocator;
...
test "Postgres: StorageKey maps field names to distinct column names" {
    const allocator = testing.allocator;
    var drv = connect(allocator) catch |err| return skipIfNoServer(err);
    defer drv.close();

    _ = drv.exec("DROP TABLE IF EXISTS pg_storage_account", &.{}) catch {};
    defer _ = drv.exec("DROP TABLE IF EXISTS pg_storage_account", &.{}) catch {};

    const PgStorageAccount = schema("PgStorageAccount", .{
        .table_name = "pg_storage_account",
        .fields = &.{
    ...
    const graph = comptime buildGraph(&.{PgStorageAccount});
    const infos = graph.types;
    try Client.createAllTables(std.testing.allocator, infos, drv.asDriver());
    var client = Client.makeClient(infos, allocator, drv.asDriver());
    const preds = client.pg_storage_account.predicates;

    // DDL created the physical columns, not the Zig field names.
    {
        var rows = try drv.query("SELECT user_name, email_address, login_count FROM pg_storage_account", &.{});
        rows.deinit();
    }
    // The index definition references the mapped column.
    {
        var rows = try drv.query(
            "SELECT indexdef FROM pg_indexes WHERE tablename = 'pg_storage_account'",
            &.{},
        );
        defer rows.deinit();
    ...
    var b1 = try client.pg_storage_account.Create();
    defer b1.deinit();
    ...
    var b2 = try client.pg_storage_account.Create();
    defer b2.deinit();
    ...
    {
        var rows = try drv.query(
            "SELECT user_name, email_address, login_count FROM pg_storage_account WHERE user_name = $1",
            &.{.{ .string = "alice" }},
        );
        defer rows.deinit();
        const row = rows.next() orelse return error.NoRow;
    ...
    {
        var q = client.pg_storage_account.Query();
        defer q.deinit();
...

    {
        var q = client.pg_storage_account.Query();
        defer q.deinit();
        _ = try q.OrderBy(&.{sql.OrderAsc("userName")});
        var found = try q.All();
        defer {
            for (found.items) |*e| zent.codegen.deinitEntity(infos, infos[0], e, allocator);
            found.deinit();
    ...
    {
        var u = client.pg_storage_account.Update();
        defer u.deinit();
    ...
    {
        var d = client.pg_storage_account.Delete();
        defer d.deinit();
...
test "Postgres: queryTargetsByValue traverses UUID-keyed parents" {
    const allocator = testing.allocator;
    var drv = connect(allocator) catch |err| return skipIfNoServer(err);
    defer drv.close();

    // `field.UUID` types the FK column as a native PostgreSQL UUID, while the
    // driver binds textual parameters. Raw DDL is used here only to pin the
    // exact column type for this parameter-typing test; the DDL the library
    // generates for a UUID primary key is covered by the createAllTables test
    // below.
    const ItemBase = schema("PgUqvItem", .{
        .fields = &.{ field.String("model"), field.UUID("pg_uqv_user_id") },
    });
    const UserBase = schema("PgUqvUser", .{
        .fields = &.{ field.UUID("id"), field.String("name") },
        .edges = &.{edge.To("items", ItemBase).Field("pg_uqv_user_id")},
    });
    const infos = comptime buildGraph(&.{ UserBase, ItemBase }).types;
    const user_info = infos[0];
    const item_info = infos[1];

    _ = try drv.exec("DROP TABLE IF EXISTS pg_uqv_item CASCADE", &.{});
    _ = try drv.exec("DROP TABLE IF EXISTS pg_uqv_user CASCADE", &.{});
    defer {
        _ = drv.exec("DROP TABLE IF EXISTS pg_uqv_item CASCADE", &.{}) catch {};
        _ = drv.exec("DROP TABLE IF EXISTS pg_uqv_user CASCADE", &.{}) catch {};
    }
    _ = try drv.exec("CREATE TABLE pg_uqv_user (id UUID PRIMARY KEY, name TEXT NOT NULL)", &.{});
    _ = try drv.exec("CREATE TABLE pg_uqv_item (id SERIAL PRIMARY KEY, model TEXT NOT NULL, pg_uqv_user_id UUID NOT NULL)", &.{});
    ...
    for (users, 0..) |uid, i| {
        var b = try client.pg_uqv_user.Create();
        defer b.deinit();
    ...
    for (items) |item| {
        var b = try client.pg_uqv_item.Create();
        defer b.deinit();
        _ = try b.setFieldValue("pg_uqv_user_id", item[0]);
        _ = try b.setFieldValue("model", item[1]);
        var e = try b.Save();
        defer zent.codegen.deinitEntity(infos, item_info, &e, allocator);
    }

    // `WHERE pg_uqv_user_id IN ($1)` binds $1 as text against a native UUID
...
test "Postgres: createAllTables keeps a UUID primary key typed as UUID" {
    const allocator = testing.allocator;
    var drv = connect(allocator) catch |err| return skipIfNoServer(err);
    defer drv.close();
    ...
    const DocBase = schema("PgUuidDoc", .{
        .fields = &.{ field.UUID("id"), field.String("title") },
    });
    const infos = comptime buildGraph(&.{DocBase}).types;
    const doc_info = infos[0];

    _ = try drv.exec("DROP TABLE IF EXISTS pg_uuid_doc CASCADE", &.{});
    defer _ = drv.exec("DROP TABLE IF EXISTS pg_uuid_doc CASCADE", &.{}) catch {};
...

    {
        var rows = try drv.query(
            "SELECT data_type FROM information_schema.columns " ++
                "WHERE table_name = 'pg_uuid_doc' AND column_name = 'id'",
            &.{},
        );
        defer rows.deinit();
        const row = rows.next() orelse return error.NoRow;
        try testing.expectEqualStrings("uuid", row.getText(0).?);
    }

    var client = Client.makeClient(infos, allocator, drv.asDriver());
    var b = try client.pg_uuid_doc.Create();
    defer b.deinit();
    _ = try b.setFieldValue("id", "01920000-0000-7000-8000-0000000000f1");
...
test "Postgres: checkSchema reports a missing M2M junction table" {
    ...
    const ZentJcPgMember = struct {
        pub const schema_name = ZentJcPgMemberBase.schema_name;
        pub const fields = ZentJcPgMemberBase.fields;
        pub const edges = &.{edge.To("tags", ZentJcPgTagBase)};
        pub const indexes = ZentJcPgMemberBase.indexes;
    };
    const ZentJcPgTag = struct {
        pub const schema_name = ZentJcPgTagBase.schema_name;
        pub const fields = ZentJcPgTagBase.fields;
        pub const edges = &.{edge.To("members", ZentJcPgMemberBase)};
        pub const indexes = ZentJcPgTagBase.indexes;
    };
    const graph = comptime buildGraph(&.{ ZentJcPgMember, ZentJcPgTag });
    const infos = graph.types;
    ...
    // Dropped out of band: reported, and the read-breaking gate fails on it.
    _ = try drv.exec("DROP TABLE zent_jc_pg_member_zent_jc_pg_tag", &.{});
    {
        const drifts = try migrate.checkSchema(allocator, drv.asDriver(), infos);
        defer migrate.freeSchemaDrift(allocator, drifts);
        try testing.expectEqual(@as(usize, 1), drifts.len);
        try testing.expectEqual(migrate.SchemaDrift.Kind.missing_junction_table, drifts[0].kind);
        try testing.expectEqualStrings("zent_jc_pg_member_zent_jc_pg_tag", drifts[0].table);
        try testing.expectEqualStrings("", drifts[0].column);
        try testing.expect(std.mem.indexOf(u8, drifts[0].index_detail, "zent_jc_pg_member <-> zent_jc_pg_tag") != null);
        try testing.expect(std.mem.indexOf(u8, drifts[0].index_detail, "database has no relation named zent_jc_pg_member_zent_jc_pg_tag") != null);
        try testing.expect(drifts[0].breaksReads());
        ...
        if (drv.query("SELECT zent_jc_pg_member_id, zent_jc_pg_tag_id FROM zent_jc_pg_member_zent_jc_pg_tag", &.{})) |rows| {
            var r = rows;
            r.deinit();
            return error.TestUnexpectedResult;
        } else |_| {}
