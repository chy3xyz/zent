...
- Version: **v0.71.0** (package version synced to tags — see `docs/RELEASING.md`).

## Commands

- `zig build` — build the library and example executables
- `zig build test` — run unit tests (456 tests, 0 leaks; leaks fail the run; count grows when libpq/libmariadb headers are present)
...
`baseline` counts move with this: unit 456, integration 234 passed + 3 skipped
